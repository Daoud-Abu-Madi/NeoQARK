...
class FilePermissions(FileContentsPlugin):
    ...
    def run(self):
        """
        Runs regex searches to detect world-readable and world-writable file permissions in the source code.
        If any matching patterns are found, an issue is logged.
        """
        if run_regex(self.file_path, WORLD_READABLE):
            self.issues.append(Issue(
                ...
                description=WORLD_READABLE_DESCRIPTION,
        ...
        if run_regex(self.file_path, WORLD_WRITEABLE):
            self.issues.append(Issue(
                ...
                severity=self.severity,
                description=WORLD_WRITEABLE_DESCRIPTION,
                file_object=self.file_path
            ))
