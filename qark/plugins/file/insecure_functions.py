...
class InsecureFunctions(CoroutinePlugin):
    """
    This plugin checks if any insecure functions, such as 'call', are being used without proper permission checks,
    which may lead to vulnerabilities in the content provider API.
    """
    def __init__(self):
    ...
    def run_coroutine(self):
        """
        Scans through the method declarations and checks for insecure functions that could result in data exposure
        or security issues due to insufficient permission checks.
        """
        while True:
            _, node = (yield)
