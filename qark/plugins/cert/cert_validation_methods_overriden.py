...
class CertValidation(CoroutinePlugin):
    """
    This plugin checks if a method in `CERT_METHODS` is overridden with an insecure version that usually does not verify
    SSL connections.
    """
    def __init__(self):
        super().__init__(category="cert", name="Certification Validation")
        self.severity = Severity.WARNING

    async def run_coroutine(self):
        while True:
            _, method_declaration = await self.yield_data()
    ...
    def _check_server_trusted(self, cert_method, current_file):
        """
        Determines if the `checkServerTrusted` method is overridden with either no function body (defaults to allow)
        ...
        elif len(cert_method.body) == 1 and isinstance(cert_method.body[0], ReturnStatement):
    ...
    def _on_received_ssl_error(self, cert_method, current_file):
        """
        Determines if the `onReceivedSslError` method is overridden with a body of `handler.proceed()` which will
        drop SSL errors.
        """
        if cert_method.body and len(cert_method.body) == 1 and isinstance(cert_method.body[0], StatementExpression):
